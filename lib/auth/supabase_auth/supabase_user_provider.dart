...
class MixItUPSupabaseUser extends BaseAuthUser {
  MixItUPSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> mixItUPSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MixItUPSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
